...
# Install mkcert and create certs for services
mkcert -install
mkcert pterodactyl.test traefik.pterodactyl.test wings.pterodactyl.test minio.pterodactyl.test s3.minio.pterodactyl.test

# Because we're doing Docker-in-Docker we actually need these paths to line up correctly with the host system.
...
if [ ! -f "/etc/hosts" ]; then
  ...
  for DOMAIN in "pterodactyl.test" "traefik.pterodactyl.test" "wings.pterodactyl.test" "minio.pterodactyl.test" "s3.minio.pterodactyl.test"
  do
